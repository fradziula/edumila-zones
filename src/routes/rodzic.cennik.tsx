...
export const Route = createFileRoute("/rodzic/cennik")({
  head: () => ({ meta: [{ title: "Cennik — EduMila" }, { name: "description", content: "Ceny lekcji indywidualnych, grupowych i pakietów." }] }),
  component: Cennik,
});
...
function Cennik() {
  return (
    <>
      <PageHero eyebrow="Cennik" tone="parent" title="Jasne ceny, bez gwiazdek." subtitle="Płatność po lekcji lub z góry za pakiet. Faktury — na życzenie." />
      <Section title="Pojedyncze lekcje">
        <div className="grid gap-5 md:grid-cols-3">
          {single.map((p) => (
            <div key={p.t} className={`card-surface p-7 relative ${p.featured ? "glow-parent ring-1 ring-parent/40" : ""}`}>
              {p.featured && <span className="absolute -top-3 left-6 text-xs px-3 py-1 rounded-full bg-parent text-background font-medium">Polecane</span>}
          ...
          {packs.map((p) => (
            <div key={p.t} className={`card-surface p-7 relative ${p.featured ? "glow-parent ring-1 ring-parent/50" : ""}`}>
              ...
              <ul className="mt-5 space-y-2 text-sm">
                <li className="flex gap-2"><Check size={16} className="text-parent mt-0.5"/> Ważność 6 miesięcy</li>
                <li className="flex gap-2"><Check size={16} className="text-parent mt-0.5"/> Możliwość przesunięcia terminu</li>
              </ul>
            </div>
          ))}
        </div>
        <div className="mt-10 flex flex-wrap gap-3">
          <Link to="/kontakt" className="inline-flex px-5 py-3 rounded-full bg-foreground text-background font-medium">Zarezerwuj lekcję</Link>
          <Link to="/rodzic/regulamin" className="inline-flex px-5 py-3 rounded-full border border-border">Regulamin</Link>
        </div>
      </Section>
    </>
  );
